...
public class healthdata_copd extends AppCompatActivity {

    Button return_b;
    Button next;
    RadioButton b1,b2,b3,b4,b5,b6,b7,b8,b9,b10,b11,b12,b13,b14;

    @SuppressLint("MissingInflatedId")
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.healthdata_copd);
        ...
        return_b.setOnClickListener(new View.OnClickListener() {
            public void onClick(View view) {
                Intent intent = new Intent(getApplication(), healthdata_highblood.class);
    ...
    public void calculatePercentage() {
        ...
        if(b11.isChecked()) check++;
        if(b12.isChecked()) check++;
        if(b13.isChecked()) check++;
        if(b14.isChecked()) check++;


        if(total>0){
            int percentage=(check*100)/total;
            SharedPreferences.Editor editor= sharedPreferences.edit();
            editor.putInt("copd_percentage",percentage);
            editor.apply();

            //다음 페이지 이동
            Intent intent=new Intent(healthdata_copd.this, healthdata_simhyal.class);
            startActivity(intent);
        }
