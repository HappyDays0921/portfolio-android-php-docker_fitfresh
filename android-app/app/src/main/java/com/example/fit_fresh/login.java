                    ...
                    public void onResponse(String response) {
                        try {
                            ...
                            if (success) {//회원등록 성공한 경우
                                ...
                                String userAge=jasonObject.get("userAge").toString();

                                //사용자에게 짧은 메세지를 표시하는 안드로이드 Toast 생성하여 "로그인 성공" 메시지 표시
                                Toast.makeText(getApplicationContext(), "로그인 성공", Toast.LENGTH_SHORT).show();
                                Intent intent = new Intent(login.this, MainActivity.class);

                                //SharedPreferences로 이름과 나이 넘겨버리기
                                SharedPreferences sharedPreferences= getSharedPreferences("INFO",MODE_PRIVATE);
                                String name= sharedPreferences.getString("userName","999");
                                String age= sharedPreferences.getString("userAge", "999");
